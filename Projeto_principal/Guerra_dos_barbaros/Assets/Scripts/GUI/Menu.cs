...
public class Menu : MonoBehaviour {
	...
	void Start()
	{
		...
		menu_opcao.transform.FindChild ("Slider").FindChild ("qualidade").GetComponent<Text> ().text = "Alto";
	}
	...
	public void  qualidade(Slider slider)
	{
		if (slider.value >= 0 & slider.value <= 0.375) {
			...
			slider.transform.FindChild("qualidade").GetComponent<Text>().text = "Baixo";
			Debug.Log(QualitySettings.GetQualityLevel());
				}
		if (slider.value > 0.375 & slider.value <= 0.75) {
						slider.value = 0.5f;
			QualitySettings.SetQualityLevel(3);
			slider.transform.FindChild("qualidade").GetComponent<Text>().text = "Medio";
			Debug.Log(QualitySettings.GetQualityLevel());
				}
		if (slider.value > 0.75 & slider.value <= 1) {
						slider.value = 1;
			QualitySettings.SetQualityLevel(5);
			slider.transform.FindChild("qualidade").GetComponent<Text>().text = "Alto";
			Debug.Log(QualitySettings.GetQualityLevel());
				}
	}
